    ...
    try:
       ...
       # 4. [ السجل المركزي - سوبابيس ]
        ...
        for i, q in enumerate(selected_questions):
            ...
            if current_style == 'اختيارات 📊':
                close_tasks = [bot.stop_poll(c, active_quizzes[c]['last_poll_id']) 
                               for c in chats_to_broadcast if active_quizzes.get(c, {}).get('last_poll_id')]
...
async def unified_answer_checker(m: types.Message):
    ...
    if cid in active_quizzes and active_quizzes[cid].get('active'):
        ...
        if is_answer_correct(user_text, correct_ans):
            ...
            if t < 3.0: s_title, extra_pts = "⚡ (خارق الصمت)", 100
            elif t < 4.0: s_title, extra_pts = "🚀 (القناص السريع)", 60
            elif t < 7.0: s_title, extra_pts = "🏹 (المتمكن)", 30
            ...
            if quiz.get('mode') == 'السرعة ⚡':
                ...
                logging.info(f"⚡ حسم عالمي: {m.from_user.first_name} أغلق السؤال في جميع المجموعات.")
