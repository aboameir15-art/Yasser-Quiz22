...
async def deep_privacy_scan(user, bot):
    ...
    female_keywords = [
        'بنت', 'ام ', 'الانسة', 'الاخت', 'مدام', 'ة', 'امة', 
...
async def send_creative_results(chat_id, correct_ans, winners, group_scores, is_public=False, mode="السرعة ⚡", group_names=None, losers=None):
    ...
    if is_public and group_scores:
        ...
        for i, g in enumerate(sorted_groups):
            ...
            # --- [ هنا تضع الكود الجديد ] ---
            for player in g['players']:
                p_id = player.get('id')
                p_name = player.get('name', 'لاعب مجهول')
                p_user = player.get('user_name') # اليوزر (بدون @)
                p_home = str(player.get('home_cid', ''))
                current_chat = str(chat_id)
                ...
                # ⚖️ تطبيق قواعد الحماية + الضيافة
                if p_is_female:
                    p_link = f"<b>{p_name}</b>"
                elif current_chat != p_home:
                    if p_user:
...
def get_profile_keyboard(user_id):
    ...
    keyboard = InlineKeyboardMarkup(row_width=2)
    keyboard.add(
       # ع_د اختصار لـ "عرض الدول"
       InlineKeyboardButton("🚩 : إضافة دولتي", callback_data=f"prof:list:{user_id}"),
       InlineKeyboardButton("❌ : إغلاق", callback_data="close_card")
    )
    return keyboard
...
def get_countries_keyboard(user_id: int):
    ...
    countries = [
        ...
        ("العراق", "🇮🇶"), ("الأردن", "🇯🇴"), ("فلسطين", "🇵🇸"), ("المغرب", "🇲🇦")
    ]
    buttons = []
    for name, flag in countries:
        # ح_د اختصار لـ "حفظ الدولة" - ندمج العلم والاسم لتوفير المساحة
        buttons.append(InlineKeyboardButton(text=f"{name} {flag}", callback_data=f"prof:save:{name}:{flag}:{user_id}"))
    
    keyboard.add(*buttons)
    keyboard.row(InlineKeyboardButton(text="⬅️ رجوع للبروفايل", callback_data=f"prof:back:{user_id}"))
    return keyboard
...
# ==========================================
# 2️⃣ المعالج الرئيسي للأوامر (انسحاب من المسابقة)
# ==========================================
@dp.message_handler(lambda m: m.text and m.text.strip() in ["زدني قف", "زدني توقف", "انسحاب"])
async def stop_quiz_handler(m: types.Message):
    cid, uid = m.chat.id, m.from_user.id
    user_text = m.text.strip()
    u_name = m.from_user.first_name

    # 🛑 [الإيقاف الكامل] - صلاحيات العميد!
    if user_text in ["زدني قف", "زدني توقف"]:
        quiz = active_quizzes.get(cid)
        if not quiz: 
            return await m.reply("عذراً.. القاعة فارغة حالياً ولا توجد محاضرة قائمة لإيقافها! 🏫")
        
        owner_id = quiz.get('owner_id') or quiz.get('created_by')
        if uid != owner_id:
            return await m.reply(f"يا {u_name}، أنت لا تملك صلاحيات 'عميد الكلية' لإلغاء هذا الاختبار! يرجى العودة لمقعدك. 👨‍🏫")
        ...
        academic_stops = [
            f"⚠️ رفعت الأقلام وجفت الصحف! بطلب من {u_name}، تم إلغاء الاختبار النهائي فوراً. 📄",
            f"🚫 تم إخلاء القاعة! {u_name} قرر إنهاء الفصل الدراسي الآن. تم تنظيف السجلات الأكاديمية. 🧹",
            "🏫 انتهت المحاضرة! تم إيقاف المسابقة بقرار إداري صارم. نراكم في الدور الثاني! ✨"
        ]
        await m.answer(random.choice(academic_stops))

    # 🏃 [الانسحاب] - ترك قاعة الامتحان بصمت
    elif user_text == "انسحاب":
        if cid not in active_quizzes: 
            return await m.reply("لا يمكنك الانسحاب من اختبار لم يبدأ بعد! ركز يا طالب. 📝")

        # إزالة المجموعة من كشف الحضور العام
        if 'participants_ids' in active_quizzes[cid]:
            p_list = active_quizzes[cid]['participants_ids']
            if cid in p_list: p_list.remove(cid)
        
        active_quizzes[cid]['active'] = False
        active_quizzes.pop(cid, None)

        withdraw_academic = [
            f"📉 الطالب {u_name} قرر الانسحاب من المادة! سيتم تسجيل 'غائب' لهذه المجموعة. 🚶‍♂️",
            f"🏫 يبدو أن الأسئلة تجاوزت المستوى الأكاديمي لـ {u_name}.. تم قبول طلب الانسحاب بصمت. 🤫",
            "👨‍🎓 تم سحب أوراق هذه المجموعة من اللجنة. المنافسة مستمرة بين الدوافير في القاعات الأخرى! 🔥"
        ]
        await m.answer(random.choice(withdraw_academic))
...
async def cmd_show_profile_global(message: types.Message):
    ...
    await asyncio.sleep(80) # الانتظار لمدة 60 ثانية
    try:
...
async def private_start_handler(message: types.Message):
    ...
    welcome_msg = (
        ...
        f"    ❃┅┅┅┄┄┄┈•❃•┈┄┄┄┅┅┅❃\n"
        f"💎 <b>ميزة خاصة:</b> يمكنك جمع النقاط واستبدالها بجوائز حقيقية داخل المتجر!  اكتب تحويل  لتحويل نقاطك الى لاعب اخر\n\n"
...
async def unified_profile_manager(c: types.CallbackQuery):
    data = c.data.split(':')
    action = data[1] # list, save, back
    user_id = int(data[-1]) # الـ ID دائماً في الأخير
    
    # 🕵️ فحص الهوية: ممنوع أي طالب يعبث بملف زميله!
    if c.from_user.id != user_id:
        return await c.answer("⚠️ عذراً.. هذه اللوحة خاصة بصاحب الحساب فقط! 👨‍🏫", show_alert=True)

    # 1️⃣ حالة عرض قائمة الدول (list)
    if action == "list":
        await c.message.edit_text(
            "🌍 **القاعة الدولية:**\nيرجى اختيار علم دولتك ليتم توثيقه في سجلك الأكاديمي:",
            reply_markup=get_countries_keyboard(user_id),
            parse_mode="Markdown"
        )
        await c.answer("جاري جلب القائمة..")

    # 2️⃣ حالة حفظ الدولة (save)
    elif action == "save":
        country_name = data[2]
        country_flag = data[3]
        ...
        try:
            ...
            await c.answer(f"✅ تم التوثيق: {country_name} {country_flag}", show_alert=False)
            
            # العودة التلقائية للبروفايل بعد الحفظ لإظهار النتيجة
            await return_to_main_profile(c, user_id)
            
        except Exception as e:
            logging.error(f"Error saving country: {e}")
            await c.answer("🚨 فشل في تسجيل البيانات، حاول لاحقاً.", show_alert=True)

    # 3️⃣ حالة الرجوع (back)
    elif action == "back":
        await return_to_main_profile(c, user_id)
        await c.answer("العودة للمكتب..")

# 🏛️ دالة مساعدة لإعادة بناء البروفايل (توفيراً للكود)
async def return_to_main_profile(c, user_id):
    try:
        res = supabase.table("users_global_profile").select("*").eq("user_id", user_id).single().execute()
        if res.data:
            # افترضت وجود هذه الدوال لديك لنسق البروفايل
            from bot_file import format_profile_card # تأكد من المسار
            text = await format_profile_card(res.data, user_id)
            await c.message.edit_text(text, reply_markup=get_profile_keyboard(user_id), parse_mode="HTML")
    except:
        await c.message.edit_text("🪪 **لوحة البروفايل**", reply_markup=get_profile_keyboard(user_id))
...
# ============================================================
# [1] معالج عرض المسابقات - نظام العدادات الحية والمطور الصديق
# ============================================================
@dp.message_handler(lambda message: message.text in ["مسابقة", "مسابقه"])
...
async def show_quizzes(obj):
    ...
    # 🔄 إشعار جاري التحديث
    if is_callback:
        await obj.answer("🔄 جاري تحديث العدادات الذكية..")

    try:
        # 📊 [إحصائيات سريعة] - حساب عدد المسابقات وعدد الأسئلة الإجمالي
        # جلب المسابقات
        res_quizzes = supabase.table("saved_quizzes").select("id").eq("created_by", u_id).execute()
        count_quizzes = len(res_quizzes.data) if res_quizzes.data else 0
        
        # جلب إجمالي الأسئلة التي كتبها هذا المستخدم
        res_questions = supabase.table("questions").select("id").eq("created_by", u_id).execute()
        count_questions = len(res_questions.data) if res_questions.data else 0
        ...
        if count_quizzes == 0:
            msg_empty = (
                f"👋 **أهلاً بك يا  {u_name}!**\n"
                "━━━━━━━━━━━━━━━━━━\n"
                "سجلاتك الأكاديمية فارغة تماماً.. 📚\n"
                "ابدأ الآن بوضع بصمتك العلمية! ✨"
            )
            kb.add(InlineKeyboardButton("✨ ➕ إنشاء مسابقة جديدة ➕ ✨", callback_data=f"start_create_quiz_{u_id}"))
            
            if is_callback: return await obj.message.edit_text(msg_empty, reply_markup=kb, parse_mode="Markdown")
            return await obj.reply(msg_empty, reply_markup=kb, parse_mode="Markdown")

        # ✅ حالة وجود بيانات - تصميم الأزرار بالعدادات الحية
        title = (
            f"🎁 **مكتبة المسابقات - د. {u_name}:**\n"
            "━━━━━━━━━━━━━━━━━━\n"
            f"📜 عدد المسابقات الموثقة: **{count_quizzes}**\n"
            f"❓ إجمالي بنك الأسئلة: **{count_questions}** سؤالاً\n"
            "━━━━━━━━━━━━━━━━━━"
        )

        # بناء القائمة بأيقونات متوازنة
        res_data = supabase.table("saved_quizzes").select("*").eq("created_by", u_id).execute()
        for q in res_data.data:
            q_name = q.get('quiz_name', 'بدون عنوان')
            kb.add(InlineKeyboardButton(f"💠 ✨【 {q_name} 】✨ 💠", callback_data=f"manage_quiz_{q['id']}_{u_id}"))
        
        # أزرار التحكم السفلي مع العدادات الحية (التأثير البصري)
        kb.row(
            InlineKeyboardButton(f"➕ إضافة مسابقة 📥", callback_data=f"start_create_quiz_{u_id}"),
            # العداد يظهر هنا بشكل احترافي
            InlineKeyboardButton(f"📊 إحصائياتي [ {count_questions} ] 📈", callback_data=f"my_stats_{u_id}")
        )
        
        kb.add(InlineKeyboardButton("🏮 ✖️ إغلاق السجل الأكاديمي ✖️ 🏮", callback_data=f"close_{u_id}"))
        ...
        logging.error(f"Error in Smart Counters UI: {e}")
        await obj.answer("🚨 عذراً ، هناك خلل في مزامنة العدادات!", show_alert=True)
...
async def handle_secure_actions(c: types.CallbackQuery, state: FSMContext):
    try:
        ...
        # --- [ نظام تشغيل المسابقات: التحقق الأمني ] ---
            ...
            try:
                # نتحقق من وجود المجموعة وأن حالتها 'active' حصراً
                check_gp = supabase.table("groups_hub").select("status").eq("group_id", cid).single().execute()
                if check_gp.data and check_gp.data.get('status') == 'active':
                    is_active_group = True
            except Exception as e:
                logging.error(f"⚠️ خطأ فحص HUB: {e}")

            # 🛑 [ قرار حارس البوابة ]
            if not is_active_group:
                # رسالة تنبيه أكاديمية موجهة
                msg = "🚫 هذه المجموعة غير مقيدة في سجلاتنا الأكاديمية (غير مفعلة).\n\nيرجى إرسال كلمة ( تفعيل ) لتقديم طلب الاعتماد والمباشرة!"
                return await c.answer(msg, show_alert=True)

            # --- [ استمرار الإجراءات للمجموعات المعتمدة ] ---
            res = supabase.table("saved_quizzes").select("*").eq("id", quiz_id).single().execute()
            q_data = res.data
            
            if not q_data: 
                return await c.answer("❌ عذراً.. المسابقة المطلوبة غير موجودة في الأرشيف!")

            # حذف اللوحة (تنظيف القاعة)
            try: await c.message.delete()
            except: pass

            if q_data.get('is_public'):
                # 🌐 إطلاق الإذاعة العامة (بأمان تام)
                await c.answer("🌐 جاري إطلاق الإذاعة العامة للمجموعات المعتمدة...")
                await start_broadcast_process(c, quiz_id, user_id)
            else:
                # 📍 مسار التشغيل المحلي للمجموعة
                if q_data.get('is_bot_quiz'):
                    asyncio.create_task(engine_bot_questions(cid, q_data, u_name))
                else:
                    asyncio.create_task(engine_user_questions(cid, q_data, u_name))
...
async def run_universal_logic(chat_id, questions, quiz_data, owner_name, engine_type):
    ...
    for i, q in enumerate(questions):
        ...
        # --- [ نظام التلميح الذكي - يعطل تلقائياً في نمط الاختيارات ] ---
        normal_hint_str = ""
        # 💡 لا نولد التلميح إلا إذا كان "الذكاء" مفعل والنمط "نصي" فقط
        if quiz_data.get('smart_hint') and quiz_data.get('quiz_style') != 'اختيارات 📊':
        ...
        for i, q in enumerate(selected_questions):
            ...
            for cid in chats_to_broadcast:
                active_quizzes[cid] = {
                    ...
                    "ans": ans,
                    "winners": [],
                    "losers": [], # تأكد من تصفير قائمة المخطئين أيضاً
                    "start_time": start_q_time, # 👈 أضف هذا السطر هنا لكي يعمل الرادار
                    "quiz_style": current_style,
...
async def unified_answer_checker(m: types.Message):
    ...
    if cid in active_quizzes and active_quizzes[cid].get('active'):
        ...
        if is_answer_correct(user_text, correct_ans):
            # 🏁 [الحل الجذري]: سحب الوقت من 'start_time' المسجل في الرادار
            # إذا لم يجد 'start_time' سيستخدم الوقت الحالي (لتجنب Fatal Error)
            start_t = quiz.get('start_time')
            if not start_t:
                start_t = datetime.now()
            
            # حساب السرعة بالملي ثانية بدقة
            resp_t = round((datetime.now() - start_t).total_seconds(), 3)
            
            # 🏆 منطق الألقاب بناءً على سرعة أثير المطور
            if resp_t < 2.0: 
                s_title, extra_pts = "⚡ (صاعقة)", 100
            elif resp_t < 5.0: 
                s_title, extra_pts = "🚀 (سريع)", 80
            else: 
                s_title, extra_pts = "🧠 (ذكي)", 30
            
            # تحديد النقاط (إذاعة عامة أو مسابقة خاصة)
            ...
            def save_to_db(p, t_title, type_str):
                try:
                    supabase.table("answers_log").insert({
                        ...
                        "chat_id": cid,
                        "group_name": m.chat.title or "إذاعة عامة",
                        "user_id": uid, 
                        "user_name": m.from_user.first_name,
                        "answer_text": f"{user_text} {t_title}",
                        "is_correct": True, 
                        "points_earned": p,
            ...
            # تنفيذ الحفظ في الخلفية لضمان سرعة البوت
...
if __name__ == '__main__':
    ...
    # 2. تشغيل سيرفر الويب والمنبه في "خيط" مستقل
    def start_background_web():
        ...
        # دالة الإبلاغ عن الاستيقاظ
        async def notify_startup(url):
            await asyncio.sleep(10) # ننتظر قليلاً حتى يستقر البوت
            await send_log("System_Startup", f"🚀 البوت استيقظ الآن وهو جاهز!\n🔗 الرابط: {url}")

        loop.create_task(site.start())
        
        # جلب رابط السيرفر وتفعيل المنبه + التنبيه
        app_host = os.environ.get('RENDER_EXTERNAL_HOSTNAME')
        if app_host:
            app_url = f"https://{app_host}/" if "onrender.com" in app_host else f"https://{app_host}.onrender.com/"
            loop.create_task(self_ping())   # المنبه الداخلي لضمان الـ 24/7
            loop.create_task(notify_startup(app_url)) # إرسال نغزة الاستيقاظ لجروبك
            
        print(f"🚀 Keep-alive & Startup Notification Started on Port {port}")
        loop.run_forever()

    # تشغيل الخيط الجانبي (Background Thread)
    from threading import Thread
    Thread(target=start_background_web, daemon=True).start()

    # 3. تشغيل البوت (العملية الأساسية) مع صيد الأخطاء العالمي
    print("🤖 Bot is starting polling...")

    # صيد أي خطأ يهرب من الـ 80 سطر القديمة
    @dp.errors_handler()
    async def global_error_handler(update, exception):
        chat_id = update.message.chat.id if update and update.message else "System"
        await send_log("Global_Exception", str(exception), chat_id)
        logging.error(f"⚠️ Global Catch: {exception}")
        return True 

    try:
        executor.start_polling(dp, skip_updates=True)
    except Exception as e:
        # إذا انهار البوت تماماً، يرسل نغزة استغاثة
        asyncio.run(send_log("CRITICAL_HALT", f"❌ انهار البوت تماماً: {e}"))
        logging.critical(f"❌ البوت توقف: {e}")
