...
async def stop_quiz_handler(m: types.Message):
    ...
    # 🕵️ [ الرادار المحلي ] - فحص وجود مسابقة نشطة
    if cid not in active_quizzes: 
        return await m.reply("عذراً.. القاعة فارغة! لا يوجد اختبار نشط حالياً للتحكم به. 🏫")

    quiz = active_quizzes.get(cid, {})
    is_global = quiz.get('is_global', False)
    owner_id = quiz.get('owner_id') or quiz.get('created_by')
    
    # 📊 [ فحص السجلات ] - جلب رصيد المستخدم من سوبابيس
    ans_count = 0
    try:
        res_user = supabase.table("users_global_profile").select("correct_answers_count").eq("user_id", uid).execute()
        if res_user.data:
    ...
    # 🛡️ [ ميزان القوى ] - تحديد الصلاحيات
    has_authority = False
    is_admin_dev = str(uid) == str(os.getenv("ADMIN_ID")) # المطور الملكي
    
    if is_admin_dev:
        has_authority = True

    # --- [ أ - بروتوكول المسابقات العامة ] ---
    if is_global:
        if user_text in ["زدني قف", "زدني توقف"]:
            # شرط الإيقاف العام: صاحب البث أو خبير (500+ إجابة)
            if str(uid) == str(owner_id) or ans_count >= 500:
                has_authority = True
            else:
                return await m.reply(f"⚠️ <b>عفواً يا {u_name}!</b>\nإيقاف المسابقة العامة يتطلب رتبة 'خبير عالمي' (500+ إجابة) أو أن تكون أنت مشغل البث. 📉")
        
        elif user_text == "انسحاب":
            # 🎭 [ فخ صاحب المسابقة ] - ممنوع يدق الجرس ويهرب
            if str(uid) == str(owner_id):
                funny_denials = [
                    f"😂 يا {u_name}، أنت اللي دقيت الجرس وتبي تنحاش؟ ما يصير! أنت القائد، لو تبي تنهيها قل 'زدني توقف'.",
                    f"🏃‍♂️ تبي تدق الجرس وتهرب؟ عيب عليك! أنت مشغل البث، اطلب 'توقف' لو تبي تنهي الحفلة للكل.",
                    f"🎭 نظام 'دق الجرس واهرب' ممنوع لأصحاب الفخامة! بما إنك مشغل المسابقة، قل 'زدني توقف' وأنهيها بهيبتك."
                ]
                return await m.reply(random.choice(funny_denials), parse_mode="HTML")
            
            # شروط الانسحاب للمجموعات الأخرى: مشرف أو (150+ إجابة)
            is_group_admin = False
            if chat_type in ['group', 'supergroup']:
                try:
                    member = await bot.get_chat_member(cid, uid)
                    if member.status in ['creator', 'administrator']: is_group_admin = True
                except: pass
            
            if is_group_admin or ans_count >= 150:
                has_authority = True
            else:
                return await m.reply(f"⚠️ <b>عفواً!</b>\nالانسحاب متاح فقط لمشرفي القاعات أو الخبراء (150+ إجابة).\nرصيدك الحالي: {ans_count} 📉")

    # --- [ ب - بروتوكول المسابقات الخاصة ] ---
    else:
        if chat_type == 'private':
            # في شات البوت: المالك فقط
            if str(uid) == str(owner_id): has_authority = True
        else:
            # في المجموعات الخاصة: المالك أو المشرف أو (150+ إجابة)
            is_group_admin = False
            try:
                member = await bot.get_chat_member(cid, uid)
                if member.status in ['creator', 'administrator']: is_group_admin = True
            ...
            if str(uid) == str(owner_id) or is_group_admin or ans_count >= 150:
                has_authority = True
            else:
                return await m.reply(f"⚠️ التحكم في المسابقات الخاصة للمالك، المشرفين، أو الخبراء (150+) فقط!")

    # 🚀 [ مرحلة التنفيذ - تطهير السجلات والرام ] 🚀
    if has_authority:
        # 🟢 الحالة الأولى: [ الانسحاب من البث ]
        if user_text == "انسحاب":
            try:
                # 1. تنظيف المصفوفة في سوبابيس (حذف المجموعة من قائمة المشاركين)
                res = supabase.table("active_quizzes").select("id, participants_ids").contains("participants_ids", [str(cid)]).execute()
                if res.data:
                    for row in res.data:
                        current_p_ids = row.get('participants_ids', [])
                        # تصفية القائمة وحذف الآيدي
                        new_p_ids = [p for p in current_p_ids if str(p) != str(cid)]
                        supabase.table("active_quizzes").update({"participants_ids": new_p_ids}).eq("id", row['id']).execute()

                # 2. حذف السجل من جدول المشاركين الفرعي
                supabase.table("quiz_participants").delete().eq("chat_id", cid).execute()
                
                # 3. تطهير الرام فوراً
                active_broadcasts.discard(cid) # مسح من محرك البث
                if cid in active_quizzes:
                    active_quizzes[cid]['active'] = False # كسر الدورة
                    active_quizzes.pop(cid, None) # مسح من الرادار
                
                if cid in overall_scores: overall_scores.pop(cid, None)

                await m.answer(f"📉 <b>تم قبول الانسحاب!</b>\nيا {u_name}، تم حذف مجموعتك من 'قائمة البث المشترك' وتطهير الرام والقاعدة بنجاح. ✅", parse_mode="HTML")
                logging.info(f"🚶‍♂️ انسحاب نظيف للمجموعة {cid}")
            except Exception as e:
                logging.error(f"Withdraw Error: {e}")

        # 🔴 الحالة الثانية: [ الإيقاف الشامل - زدني توقف / قف ]
        else:
            try:
                # 1. إغلاق المسابقة في سوبابيس
                ...
                # 3. إغلاق الاستطلاعات (إن وجدت)
                last_poll = active_quizzes.get(cid, {}).get('last_poll_id')
                if last_poll:
                    try: await bot.stop_poll(cid, last_poll)
                    except: pass

                # 4. التصفير النهائي للرام
                active_quizzes.pop(cid, None)
                if cid in overall_scores: overall_scores.pop(cid, None)
                active_broadcasts.discard(cid)

                role_tag = "👑 [قرار سيادي]" if is_admin_dev else "🛡️ [إيقاف إداري]"
                await m.answer(f"{role_tag} <b>رفعت الأقلام!</b>\nتم إنهاء المحرك وتطهير سجلات الرام وقاعدة البيانات كلياً. 🧹", parse_mode="HTML")
                logging.info(f"🛑 إيقاف شامل للمسابقة في {cid} بواسطة {u_name}")
            except Exception as e:
...
async def engine_global_broadcast(chat_ids, quiz_data, owner_name, current_quiz_db_id=None):
    ...
    try:
        ...
        # --- [ د ] دورة البث الموحدة ---
        # 🟢 [ الخطوة الذهبية ] 🟢
        # شحن الرام بالحياة لكل المجموعات قبل دخول حلقة الأسئلة
        ...
        # الآن ندخل الحلقة بقلب قوي 💪
        for i, q in enumerate(selected_questions):
            
            # 🛑 [ فحص النبض الذكي ] 🛑
            # الآن لن يهرب المحرك لأننا شحناه بالأعلى
            is_still_running = any(active_quizzes.get(c, {}).get('active', False) for c in chats_to_broadcast)
            
            if not is_still_running:
                logging.info(f"🚨 تم رصد إشارة إغلاق عند السؤال {i+1}.. إيقاف المحرك.")
                break
            ...
            if is_hint_on:
                ans_str = str(ans).strip()
                normal_hint_str = f"مكونة من ({len(ans_str.split())}) كلمات، تبدأ بـ ( {ans_str[0]} )"
            # 🛑 [ القسم 3: زراعة محرك الانتظار (البريك) قبل البث ] 🛑
            active_cids_list = list(chats_to_broadcast)
            if active_cids_list:
                sample_cid = active_cids_list[0]
                
                # طالما أن حالة البريك مفعلة، المحرك يدخل في غفوة ولا يرسل السؤال
                while active_quizzes.get(sample_cid, {}).get('is_paused', False):
                    await asyncio.sleep(2) # نوم خفيف لعدم إرهاق السيرفر
                    
                    # فحص أمان: لو تم إلغاء المسابقة كلياً أثناء البريك
            ...
            # تسجيل معرفات الرسائل للتحكم بها لاحقاً
            ...
            # 💡 [ إضافة ذكية ]: قاموس مستقل لحفظ رصيد "المجموعة" فقط دون خلطه باللاعبين
            if 'group_total_points' not in locals():
                group_total_points = {cid: 0 for cid in all_chats}
                       
            for cid in all_chats:
                if cid in active_quizzes:
                    # --- [ تجميع الفائزين مع إصلاح المنشن ] ---
                    for winner in active_quizzes[cid].get('winners', []):
                        winner['home_cid'] = cid  
                        # صناعة منشن (رابط) آمن حتى لو لم يمتلك يوزرنيم
                        w_id = winner.get('id')
                        w_name = winner.get('name') or winner.get('username') or "لاعب"
                        winner['user_link'] = f'<a href="tg://user?id={w_id}">{w_name}</a>'
                        global_winners.append(winner)
                    
                    # --- [ تجميع المخطئين مع إصلاح المنشن ] ---
                    for loser in active_quizzes[cid].get('losers', []):
                        loser['home_cid'] = cid  
                        l_id = loser.get('id')
                        l_name = loser.get('name') or loser.get('username') or "لاعب"
            ...
            # 2️⃣ تحديث السجلات والفصل الاستراتيجي
            for cid in all_chats:
                if cid in active_quizzes:
                    active_quizzes[cid]['active'] = False
                ...
                # --- [ معالجة الإجابات الصحيحة ] ---
                for w in local_winners:
                    uid, uname, pts_earned = w['id'], w['name'], w.get('pts', 10)
                    user_mention = w.get('user_link')
                    
                    if uid not in group_scores[cid]:
                        group_scores[cid][uid] = {"name": uname, "mention": user_mention, "points": 0}
                    
                    # ✅ 1. تحديث رصيد اللاعب الفردي دائماً (لا يختلط بالمجموعة)
                    group_scores[cid][uid]['points'] += pts_earned
                    
                    # ✅ 2. تحديث رصيد المجموعة (أول إجابة فقط)
                    if not group_points_claimed:
                        final_group_pts = pts_earned + 5 # بونص أول إجابة
                        group_total_points[cid] += final_group_pts # يضاف لخزينة المجموعة المستقلة
                        group_points_claimed = True 
                        
                        try:
                            gname = group_names_map.get(cid, "مجموعة مجهولة")
                            # تحديث سوبابيس (إرسال النقاط للمجموعة + اللاعب كـ MVP)
                            await update_group_stats(cid, gname, uid, uname, final_group_pts)
                        except Exception as e:
                            logging.error(f"⚠️ خطأ سوبابيس للمجموعة: {e}")
                    else:
                        # لاعب آخر من نفس المجموعة أجاب صحيحاً:
                        # لا نعطي المجموعة نقاطاً، لكن نحفظ نقطة اللاعب في بروفايله الشخصي بسوبابيس
                        try:
                            # نفترض وجود دالة لتحديث الفرد دون المجموعة، أو تمرير 0 لنقاط المجموعة
                            await update_group_stats(cid, gname, uid, uname, 0) 
                        except: pass

                # --- [ معالجة المخطئين وإصلاح الخصم ] ---
                local_losers = active_quizzes.get(cid, {}).get('losers', [])
                for l in local_losers:
                    l_uid, l_uname, penalty = l['id'], l['name'], l.get('penalty', 5)
                    l_mention = l.get('user_link')
                    
                    if l_uid not in group_scores[cid]:
                        group_scores[cid][l_uid] = {"name": l_uname, "mention": l_mention, "points": 0}
                    
                    # 📉 الخصم من الرصيد الفردي للاعب فقط (المجموعة لا تعاقب بخطأ الفرد)
                    group_scores[cid][l_uid]['points'] -= penalty
                    
                    try:
                        gname = group_names_map.get(cid, "مجموعة مجهولة")
                        # نرسل تحديثاً لسوبابيس يخصم من اللاعب، مع تمرير 0 للمجموعة لكي لا تتضرر
                        await update_group_stats(cid, gname, l_uid, l_uname, 0)
                    except Exception as e:
                        logging.error(f"⚠️ خطأ خصم سوبابيس: {e}")

                # 3️⃣ إرسال القالب الملكي الفخم
                # 3️⃣ إرسال القالب الملكي الفخم لكل المجموعات
            res_tasks = []
            for cid in all_chats:
                res_tasks.append(send_creative_results(
                    ...
                    group_scores=group_scores[cid], # نمرر نقاط المجموعة المحددة فقط
                    is_public=True,
                    mode=quiz_data.get('mode', 'السرعة ⚡'),
                    group_names=group_names_map
                ))
            
            # 🔥 صيد مُعرفات رسائل الإجابة لضمان حذفها لاحقاً
            res_msgs = await asyncio.gather(*res_tasks, return_exceptions=True)
            for idx, rm in enumerate(res_msgs):
                current_cid = all_chats[idx]
