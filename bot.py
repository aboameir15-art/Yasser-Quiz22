...
async def stop_quiz_handler(m: types.Message):
        ...
        try:
            ...
            if res_user.data:
                ...
                if ans_count >= 150:
                    ...
                    return await m.reply(f"⚠️ يا {u_name}، صلاحية 'إيقاف المسابقة' للمشرفين أو الخبراء (150+ إجابة) فقط!\nرصيدك الحالي: {ans_count} 📉")
            else:
                return await m.reply("⚠️ لم يتم العثور على ملفك الشخصي. لا يمكنك التحكم في المحرك!")
        except Exception as e:
    ...
    if has_authority:
        ...
        if user_text == "انسحاب":
            ...
            academic_stops = [
                f"{role_tag} <b>رفعت الأقلام!</b>\nبقرار من {u_name}، تم تصفير السجلات وإخلاء القاعة. ✨",
