# --- [ 1: مكتبات بايثون القياسية - Standard Libraries ] ---
import os
import json
import time
import asyncio
...
# --- [ 2: مكتبات خارجية ومعالجة النصوص - Third-Party ] ---
import httpx  
import aiohttp
import requests
...
# --- [ 3: مكتبة Pyrogram - للمهمات المتقدمة والهمسات ] ---
...
# --- [ 4: مكتبة Aiogram - المحرك الرئيسي للبوت ] ---
...
# --- [ 5: إعداد السجلات - Logging Config ] ---
...
# --- [ 1: مخازن الذاكرة المؤقتة ] ---
# لتتبع عدد الاستخدامات لكل لاعب في السؤال الحالي
whisper_usage_limit = {} # { "chat_id_user_id": count }
# لتخزين محتوى الهمسة لكي تظهر عند الضغط على الزر
cached_whispers = {} # { "user_id": "content_text" }
# لتخزين IDs الرسائل لحذفها بعد انتهاء السؤال
active_whisper_msgs = {} # { chat_id: [msg_ids] }
whisper_usage_limit = {}  # لتتبع الـ 5 مرات
cached_whispers = {}      # لتخزين نص الهمسة المؤقت
...
# ==========================================
# --- [ 2: دوال توليد المحتوى (قوالب أثير الذكية) ] ---

def get_whisper_hint(ans):
    """توليد تلميح هيكلي (عدد كلمات، حروف، بدايات)"""
    words = ans.split()
    first_chars = [w[0] for w in words]
    total_len = len(ans.replace(" ", ""))
    return (
        f"🕵️‍♂️ همسة المساعد الذكي:\n"
        f"━━━━━━━━━━━━\n"
        f"📝 عدد الكلمات: {len(words)}\n"
        f"🔢 عدد الحروف: {total_len}\n"
        f"🔑 مفاتيح البداية: ( {' - '.join(first_chars)} )\n"
        f"━━━━━━━━━━━━\n"
        f"⚠️ اظهرها بسرعة قبل انتهاء الوقت!"
    )

def get_whisper_reveal(ans):
    """توليد كشف نصفي للإجابة (تشريح)"""
    words = ans.split()
    revealed_parts = []
    for w in words:
        reveal_limit = max(1, len(w) // 2)
        part = "".join([w[i] if i < reveal_limit else "." for i in range(len(w))])
        revealed_parts.append(part)
    return (
        f"🔓 همسة كشف المستور:\n"
        f"━━━━━━━━━━━━\n"
        f"🔎 الإجابة المقطعة: {'  '.join(revealed_parts)}\n"
        f"💡 رتب الحروف الناقصة واكتبها فوراً!\n"
        f"━━━━━━━━━━━━"
    )

async def cleanup_round_whispers(chat_id):
    # 🧹 حذف الرسائل من الشات
    if chat_id in active_whisper_msgs:
        for m_id in active_whisper_msgs[chat_id]:
            try: await bot.delete_message(chat_id, m_id)
            except: pass
        active_whisper_msgs[chat_id] = []
    
    # 🧼 تصفير العدادات والذاكرة لهذا الروم
    keys_to_del = [k for k in whisper_usage_limit if k.startswith(f"{chat_id}_")]
    for k in keys_to_del: whisper_usage_limit.pop(k, None)
    
    keys_cache_del = [k for k in cached_whispers if k.startswith(f"{chat_id}_")]
    for k in keys_cache_del: cached_whispers.pop(k, None)
...
async def get_ultra_smart_options(question_text, category_name, correct_ans):
    try:
        ...
        patterns = {
            ...
            'history_war': [
                'عام', 'العام', 'سنه', 'السنه', 'سنة', 'السنة', 'تاريخ', 'التاريخ', 'قرن', 'القرن', 
                'ميلادي', 'الميلادي', 'هجري', 'الهجري', 'معركه', 'المعركه', 'معركة', 'المعركة', 
                'غزوه', 'الغزوه', 'غزوة', 'الغزوة', 'ثوره', 'الثوره', 'ثورة', 'الثورة', 'حضاره', 
                'الحضاره', 'حضارة', 'الحضارة', 'امبراطور', 'الامبراطور', 'ملك', 'الملك', 'سلالة', 
                'السلالة', 'عصر', 'العصر', 'حرب', 'الحرب', 'قائد', 'القائد', 'رئيس', 'الرئيس'
            ],
            
            # --- الجغرافيا والكون والطبيعة ---
            'geography_world': [
                'دوله', 'الدوله', 'دولة', 'الدولة', 'بلد', 'البلد', 'عاصمه', 'العاصمه', 'عاصمة', 
                'العاصمة', 'مدينه', 'المدينه', 'مدينة', 'المدينة', 'نهر', 'النهر', 'بحر', 'البحر', 
                'محيط', 'المحيط', 'جبل', 'الجبل', 'قاره', 'القاره', 'قارة', 'القارة', 'جزيره', 
                'الجزيره', 'جزيرة', 'الجزيرة', 'خليج', 'الخليج', 'مضيق', 'المضيق', 'مناخ', 'المناخ', 
                'تضاريس', 'التضاريس', 'غابات', 'الغابات', 'ميناء', 'الميناء', 'قرية', 'القرية', 'السهول', 'قمر', 'غابة', 'الغابة', 'صحراء', 'الصحراء', 'شلال', 'الشلال', 
                'فوهة', 'فوهه', 'البركان'
            ],
            
            # --- العلوم والتقنية والطب (التشريح الدقيق) ---
            'science_tech': [
                'كوكب', 'الكوكب', 'عنصر', 'العنصر', 'غاز', 'الغاز', 'حيوان', 'الحيوان', 'طائر', 
                'الطائر', 'نبات', 'النبات', 'مبدأ', 'المبدأ', 'العضو', 'عضو', 'المصطلح', 'مصطلح', 'جسم', 'الجسم', 'خليه', 'الخليه', 'خلية', 'الخلية', 
                'جهاز', 'الجهاز', 'مخترع', 'المخترع', 'اكتشف', 'الاكتشاف', 'فيزياء', 'الفيزياء', 
                'كيمياء', 'الكيمياء', 'مجره', 'المجره', 'مجرة', 'المجرة', 'فضاء', 'الفضاء', 
                'تلسكوب', 'التلسكوب', 'هندسة', 'الهندسة', 'معدن', 'المعدن', 'ذرة', 'الذرة', 
                'مختبر', 'المختبر', 'تقنية', 'التقنية'
            ],
            # --- العلوم الطبية والصحة (تشريح دقيق) ---
            'medical_health': [
                'عضو', 'العضو', 'مرض', 'المرض', 'دواء', 'الدواء', 'فيتامين', 'الفيتامين', 
                'هرمون', 'الهرمون', 'عصب', 'العصب', 'عظم', 'العظم', 'دم', 'الدم', 'قلب', 'القلب', 
                'دماغ', 'الدماغ', 'عضله', 'العضله', 'عضلة', 'العضلة', 'غده', 'الغده', 'غدة', 'الغدة', 
                'فيروس', 'الفيروس', 'بكتيريا', 'البكتيريا', 'علاج', 'العلاج', 'جراحة', 'الجراحة', 
                'لقاح', 'اللقاح', 'معدة', 'المعدة', 'رئة', 'الرئة'
            ],
            
            # --- الفكر والأدب والشخصيات (محرك الأدباء) ---
            'humanities': [
                'من هو', 'الشاعر', 'شاعر', 'مؤلف', 'المؤلف', 'كاتب', 'الكاتب', 'فنان', 'الفنان', 
                'رسام', 'الرسام', 'فيلسوف', 'الفيلسوف', 'رواية', 'روايه', 'الرواية', 'الروايه', 
                'قصيدة', 'قصيده', 'القصيدة', 'القصيده', 'لوحة', 'لوحه', 'اللوحة', 'اللوحه', 
                'تمثال', 'التمثال', 'مسرحية', 'مسرحيه', 'المسرحية', 'المسرحيه', 'فكر', 'الفكر', 
                'منطق', 'المنطق', 'ثقافة', 'ثقافه', 'الثقافة', 'الثقافه', 'أديب', 'الأديب'
            ],
            
            # --- العلوم الإسلامية والدينية ---
            'islamic_religion': [
                'سوره', 'سورة', 'السوره', 'السورة', 'آيه', 'آية', 'الآيه', 'الآية', 'نبي', 'النبي', 
                'رسول', 'الرسول', 'الصحابي', 'صحابي', 'تابعي', 'التابعي', 'فقيه', 'الفقيه', 
                'كتاب', 'الكتاب', 'تفسير', 'التفسير', 'حديث', 'الحديث', 'صلاه', 'صلاة', 'الصلاه', 
                'الصلاة', 'حج', 'الحج', 'زكاه', 'زكاة', 'الزكاه', 'الزكاة', 'غزوة', 'غزوه', 'الغزوة', 
                'الخليفة', 'خليفة', 'إمام', 'امام'
            ],
            
            # --- الرياضة والترفيه (محرك الحماس) ---
            'sports_ent': [
                'لاعب', 'اللاعب', 'نادي', 'النادي', 'منتخب', 'المنتخب', 'بطوله', 'بطولة', 
                'البطوله', 'البطولة', 'كاس', 'كأس', 'الكاس', 'الكأس', 'ملعب', 'الملعب', 'هدف', 'الهدف', 
                'اولمبياد', 'الاولمبياد', 'فيلم', 'الفيلم', 'مسلسل', 'المسلسل', 'ممثل', 'الممثل', 
                'مخرج', 'المخرج', 'اغنية', 'اغنيه', 'الاغنية', 'الاغنيه', 'رقم قياسي', 'دوري', 'الدوري'
            ],
            
            # --- الحساب والمنطق والرياضيات ---
            'math_logic': [
                'حساب', 'الحساب', 'رياضيات', 'الرياضيات', 'مربع', 'المربع', 'مثلث', 'المثلث', 
                'زاويه', 'زاوية', 'الزاويه', 'الزاوية', 'مجموع', 'المجموع', 'ناتج', 'الناتج', 
                'معادله', 'معادلة', 'المعادله', 'المعادلة', 'رقم', 'الرقم', 'ضعف', 'الضعف', 
                'جذر', 'الجذر', 'قسمة', 'قسمه', 'القسمة', 'ضرب', 'الضرب', 'طرح', 'جمع'
            ],
            
            # --- القياسات والكميات (رادار الأرقام) ---
            'measurements': [
                'كم عدد', 'العدد', 'كم يبلغ', 'كم تبلغ', 'ما طول', 'الطول', 'ما وزن', 'الوزن', 
                'نسبه', 'نسبة', 'النسبه', 'النسبة', 'مسافه', 'مسافة', 'المسافه', 'المسافة', 
                'الوحده', 'الوحدة', 'الواحدة', 'لقياس', 'الوحدة', 'الوحده', 'وحده', 'الحالة', 'الحاله', 'حالة', 'حاله', 'الجسيم', 'جسيم', 'القياس', 'مادة', 'ماده', 'المادة', 'الماده', 
                'الزمن', 'سرعه', 'سرعة', 'السرعه', 'السرعة', 'درجه', 'درجة', 'الدرجه', 'الدرجة',
                'عمق', 'العمق', 'مساحه', 'مساحة', 'المساحة'
            ],
            
            # --- الاقتصاد، القانون، والسياسة (المحرك السيادي) ---
            'eco_law_pol': [
                'عملة', 'عمله', 'العملة', 'العمله', 'بورصة', 'بورصه', 'البورصة', 'اقتصاد', 'الاقتصاد', 
                'شركة', 'شركه', 'الشركة', 'الشركه', 'قانون', 'القانون', 'محكمة', 'محكمه', 'المحكمة', 
                'دستور', 'الدستور', 'رئيس', 'الرئيس', 'وزير', 'الوزير', 'برلمان', 'البرلمان', 
                'انتخابات', 'الانتخابات', 'سياسة', 'سياسه', 'السياسة', 'السياسه', 'منظمة', 'المنظمة'
            ],

            # --- اللغة، الأمثال، والبلاغة ---
            'language_proverbs': [
                'مرادف', 'المرادف', 'ضد', 'الضد', 'عكس', 'العكس', 'معنى', 'المعنى', 'جمع', 'الجمع', 
                'مفرد', 'المفرد', 'مثل', 'المثل', 'حكمه', 'حكمة', 'الحكمة', 'الحكمه', 'قائل', 'القائل', 
                'كلمة', 'كلمه', 'الكلمة', 'الكلمه', 'لغة', 'لغه', 'اللغة', 'اللغه', 'لهجة', 'لهجه', 
                'اللهجة', 'بيت الشعر', 'إعراب', 'اعراب'
            ],
            
            # --- نمط الحياة والبراندات (المحرك العصري) ---
            'lifestyle_brands': [
                'اكله', 'اكلة', 'الاكلة', 'وجبه', 'وجبة', 'الوجبة', 'مطبخ', 'المطبخ', 'فاكهه', 
                'فاكهة', 'الفاكهة', 'خضار', 'الخضار', 'سياره', 'سيارة', 'السيارة', 'السياره', 
                'ماركه', 'ماركة', 'الماركة', 'شعار', 'الشعار', 'هاتف', 'الهاتف', 'تطبيق', 'التطبيق', 
                'برج', 'البرج', 'عطر', 'العطر', 'شركة تصنيع'
            ],
            
            # --- الأساطير والماورائيات ---
            'myth_folklore': [
                'اسطورة', 'اسطوره', 'الاسطورة', 'الاسطوره', 'خرافة', 'خرافه', 'الخرافة', 'الخرافه', 
                'اله', 'الإله', 'عملاق', 'العملاق', 'تنين', 'التنين', 'فلكلور', 'الفلكلور', 'كنز', 
                'وحش', 'الوحش', 'مخلوق'
            ]
...
async def send_creative_results(chat_id, correct_ans, winners, group_scores, is_public=False, mode="السرعة ⚡", group_names=None, losers=None):
 ...
 # ==========================================
# --- [ قالب النتائج النهائية للمسابقة العامة ] ---
# ==========================================        
    ...
    try:
        ...
        if group_summary:
            ...
            for i, g in enumerate(sorted_groups, 1):
                is_first = (i == 1)
                medal = "🥇 ا:ا" if is_first else "🥈 ا:ا" if i == 2 else "🥉 ا:ا" if i == 3 else "🔹 ا:ا"
                gift_msg = ""
                ...
                msg += f"📊 ا:ا الإنجاز ( <code>{round(g['rate'], 1)}%</code> ) | إجمالي: {g['total']} ن\n"
                
                # عرض تفاصيل اللاعبين
                for p in g['players'][:5]:
                    msg += f"👤 ا:ا {p['link']} ⇠ <b>{p['pts']}</b> ن ( {p['count']})\n"
                msg += "┅┅┅┅┅┅┅┅┅┅┅┅┅┅┅┅\n"
        ...
        if all_global_players:
            ...
            for i, (uid, p) in enumerate(sorted_global[:5], 1):
                icon = "🥇 ا:ا" if i == 1 else "🥈 ا:ا" if i == 2 else "🥉 ا:ا" if i == 3 else "👤 ا:ا"
                ...
                msg += f"{icon} {p['name']} ⇠ <b>{p['total_pts']}</b> ن |  {p['count']} (🧠 ا:ا {final_iq}% IQ)\n"

        if not found_any_score:
            msg = "🌍 <b>: انتهت الإذاعة !</b>\n\nلم يتم تسجيل أي نقاط ."
        else:
            msg += f"\n👥 ا:ا الأبطال: <b>{len(all_global_players)}</b> | 📋 : الأسئلة: <b>{total_q}</b>\n"
            msg += "🎁 تم إيداع 1000 نقطة هدية في محفظة ابطال المركز الاول!"
...
async def process_board_navigation(c: types.CallbackQuery):
    ...
    try:
        ...
        # عرض الخطأ في الكونسول لتسهيل تتبعه
        print(f"DEBUG ERROR: {e}")
        await c.answer("❌ عذراً.. حدث خطأ في جلب البيانات.", show_alert=True)
# ============================================================
...
# ==========================================
# 1. قسم إدارة الجلسات (State Management)
# ==========================================
# لتخزين إعدادات الصفقة المؤقتة للاعب قبل التأكيد
trade_sessions = {}

# قوائم الدوران للأزرار (Rotating Values)
...
# ==========================================
# 2. قسم الدوال الحسابية والمالية الأساسية
# ==========================================

def calculate_liquidation(entry_price, leverage, side):
    """حساب دقيق لسعر التصفية (بينانس بالملي)"""
    entry = float(entry_price)
    lev = int(leverage)
    if side == 'LONG':
        # التصفية عند هبوط السعر لدرجة تأكل الهامش
        return entry * (1 - (1.0 / lev))
    else: # SHORT
        # التصفية عند صعود السعر
        return entry * (1 + (1.0 / lev))

def generate_candle_chart(direction):
    """محاكاة بصرية للشموع اليابانية"""
    if direction == 'UP':
        return "📉 ⇠ |---🟩---| ⇠ 🚀 صعود إيجابي"
    else:
        return "📈 ⇠ |---🟥---| ⇠ 🩸 هبوط سلبي"
...
async def get_user_bank_balance(user_id):
    """جلب رصيد البنك الخاص بالتداول"""
    res = supabase.table("users_global_profile").select("bank_balance").eq("user_id", user_id).execute()
    if res.data:
        return float(res.data[0].get('bank_balance', 0))
    return 0.0

async def market_pulse_engine():
    """
    المحرك الرئيسي: يعمل في الخلفية كل 60 ثانية لتحديث الأسعار وتصفية الصفقات.
    هذا هو "القلب" الذي يجعل الأرقام تتحرك في سوبابيس.
    """
    while True:
        try:
            # 1. جلب كافة العملات من جدول المحاكاة
            ...
            for coin in coins:
                ...
                # محاكاة حركة السعر (تذبذب بين -1.5% و +1.5%)
                change_percent = random.uniform(-0.015, 0.015)
                new_price = old_price * (1 + change_percent)
                
                # تحديث المؤشرات الفنية (محاكاة ذكية)
                # EMA 50 يتحرك ببطء خلف السعر
                new_ema = (old_price * 0.1) + (float(coin.get('ema_50', old_price)) * 0.9)
                
                # RSI يتأثر بقوة الحركة (محاكاة بسيطة)
                new_rsi = float(coin.get('rsi_val', 50)) + (change_percent * 1000)
                new_rsi = max(10, min(90, new_rsi)) # حصر RSI بين 10 و 90
                ...
                # 2. فحص التصفية (Liquidation Check) لكل عملة
                await auto_liquidation_monitor(symbol, new_price)

            logging.info(f"✨ تم تحديث نبض السوق وفحص التصفية لـ {len(coins)} عملة.")

        except Exception as e:
            logging.error(f"❌ خطأ في محرك نبض السوق: {e}")
            
        # الانتظار لمدة دقيقة قبل النبضة التالية
        await asyncio.sleep(60)

async def auto_liquidation_monitor(symbol, current_price):
    """فحص الصفقات المفتوحة لهذه العملة وإغلاقها إذا ضربت سعر التصفية"""
    trades = supabase.table("active_trades")\
        .select("*")\
        .eq("symbol", symbol)\
        .eq("is_active", True)\
        .execute()
    ...
    for trade in trades.data:
        ...
        # شرط التصفية (ضرب الاستوب لوز الإجباري)
        is_liquidated = (side == 'LONG' and current_price <= liq_price) or \
                        (side == 'SHORT' and current_price >= liq_price)
        
        if is_liquidated:
            # 1. إغلاق الصفقة بخسارة كامل الهامش
            supabase.table("active_trades").update({
                "is_active": False,
                "close_price": current_price,
                "pnl": -float(trade['margin']),
                "closed_at": datetime.now().isoformat()
            }).eq("id", trade['id']).execute()
            
            # 2. إرسال إشعار للاعب (اختياري - يفضل ربطه بدالة إرسال رسائل)
            try:
                await bot.send_message(user_id, f"🩸 <b>نداء الهامش (Margin Call)!</b>\n\nتم تصفية صفقتك على #{symbol} بسبب وصول السعر إلى <code>{current_price:,.4f} $</code>.\nخسارة الهامش: {float(trade['margin']):,.2f} $", parse_mode="HTML")
            except: pass
            
# ==========================================
# 3. قسم قوالب واجهات المستخدم (Secured UI/UX)
# ==========================================

def get_market_keyboard(user_id):
    """واجهة السوق الرئيسية - مؤمنة بآيدي المستخدم"""
    markup = InlineKeyboardMarkup(row_width=3)
    markup.row(
        InlineKeyboardButton("🔥 الأكثر رواجاً", callback_data=f"market_tab:{user_id}:trending"),
...
def get_coin_keyboard(user_id, symbol):
    """واجهة اختيار نوع الصفقة لعملة محددة"""
...
def get_trade_setup_keyboard(user_id):
    """واجهة إعدادات الرافعة والهامش - قمة التأمين"""
    session = trade_sessions.get(user_id)
    if not session: return None # حماية في حال مسح الجلسة
    
    sym = session['symbol']
    side = session['side']
    
    markup = InlineKeyboardMarkup(row_width=2)
    # أزرار دوارة مربوطة بالـ user_id لمنع التلاعب بالإعدادات من شخص آخر
    markup.row(
    ...
    # زر التأكيد النهائي يحمل الآيدي لضمان عدم تنفيذ الصفقة من مبعسس
...
# ==========================================
# 2. محرك الحماية والمنطق المالي (The Engine)
# ==========================================

async def get_user_data(user_id):
    """جلب بروفايل المستخدم المالي بالكامل"""
...
async def check_financial_health(user_id, amount, action="WITHDRAW"):
    """
    نظام الحماية الصارم (The Guard):
    - لا سحب قبل تسديد الديون.
    - لا تحويل إذا كانت الصفقات المفتوحة مهددة بالتصفية.
    """
    data = await get_user_data(user_id)
    bank_bal = float(data.get('bank_balance', 0))
    debt = float(data.get('debt_balance', 0))
    
    # حساب الهامش المحجوز في الصفقات
    trades = supabase.table("active_trades").select("margin").eq("user_id", user_id).eq("is_active", True).execute().data
    locked_margin = sum(float(t['margin']) for t in trades) if trades else 0
    available_cash = bank_bal - locked_margin

    if action == "WITHDRAW":
        if debt > 0:
            return False, f"⚠️ لا يمكنك السحب! لديك دين مستحق بقيمة {debt:,.2f} $. سدد ديونك أولاً."
        if amount > available_cash:
            return False, f"⚠️ المبلغ محجوز في صفقات نشطة. المتاح فعلياً: {available_cash:,.2f} $."
    
    elif action == "BORROW":
        if debt > 0:
            return False, "⚠️ لديك قرض نشط. لا يمكنك الاقتراض مجدداً قبل السداد."
        if bank_bal < 10:
            return False, "⚠️ رصيدك ضعيف جداً للحصول على ائتمان. أودع أولاً."
            
    return True, "Success"

# ==========================================
# 3. واجهات المستخدم المؤمنة (Secured UI/UX)
# ==========================================

def get_wallet_keyboard(user_id, debt):
    """
    تأمين الأزرار عبر ربطها بآيدي المستخدم لمنع 'المبعسسين'
    """
    markup = InlineKeyboardMarkup(row_width=2)
    
    # تحويل وإيداع (مربوطة بالآيدي)
    markup.row(
    ...
    # التنقل (مربوطة بالآيدي)
    markup.add(
        InlineKeyboardButton("📋 صفقاتي", callback_data=f"active_trades_view:{user_id}"), 
        InlineKeyboardButton("🛒 السوق", callback_data=f"market_tab:trending:{user_id}")
    )
    return markup
...
def get_trades_keyboard(user_id, trades):
    ...
    for trade in trades:
        symbol = trade['symbol']
        trade_id = trade['id']
        # أزرار التحكم لكل صفقة
        markup.add(
            InlineKeyboardButton(f"🚀 تعزيز (DCA) {symbol}", callback_data=f"dca_trade:{user_id}:{trade_id}"),
            InlineKeyboardButton(f"❌ إغلاق الآن", callback_data=f"close_trade:{user_id}:{trade_id}")
        )
    # زر العودة للسوق
    markup.add(InlineKeyboardButton("🔙 العودة للسوق", callback_data=f"market_tab:{user_id}:trending"))
    return markup
...
async def get_active_trades_report(user_id):
    """تجلب الصفقات وتحسب الأرباح اللحظية لكل صفقة"""
    ...
    for trade in trades:
        ...
        # جلب السعر الحالي للحساب اللحظي
...
async def close_trade_manually(trade_id, current_price):
    """إغلاق الصفقة يدوياً وحساب الأرباح وإعادة الهامش للبنك"""
    ...
    # حساب الربح أو الخسارة بالمعادلة التي يحبها ChatGPT
    if side == 'LONG':
    ...
    pnl_amount = margin * pnl_pct * lev
    total_return = margin + pnl_amount # الهامش المسترد + الربح/الخسارة
    
    # 1. تحديث رصيد البنك (إعادة الأموال)
...
async def update_trade_ui(callback_query):
    ...
    text += f"🪙 حـجـم الـعـمـلات (الكمية): <b>{quantity:,.6f} {sym}</b>\n"
...
# ==========================================
# 5. لمسة الشريك: محاكاة التصفير (Liquidation Engine)
# ==========================================
async def on_startup(dp):
    # تشغيل محرك السوق في الخلفية كـ Task مستقلة
    asyncio.create_task(market_pulse_engine())
    print("🚀 محرك نبض السوق المباشر يعمل الآن...")
    
async def monitor_liquidation():
    """وظيفة تعمل في الخلفية لمراقبة تصفير المحفظة"""
    while True:
        # هذه الدالة يجب أن يتم استدعاؤها دورياً مع تحديث الأسعار
        # إذا وصل السعر لسعر التصفية:
        # 1. إغلاق الصفقة فوراً.
        # 2. تصفير الهامش (Margin = 0).
        # 3. إرسال إشعار: "🩸 لقد تم تصفية حسابك! خسرنا المعركة لكن لم نخسر الحرب."
        await asyncio.sleep(60)
# ==========================================
...
# --- [ الكروت ] ---
# ==========================================
@dp.message_handler(lambda message: message.text in ["تلميح", "كشف"], chat_type=[types.ChatType.GROUP, types.ChatType.SUPERGROUP])
async def whisper_system_listener(message: types.Message):
    user_id = message.from_user.id
    chat_id = message.chat.id
    cmd = message.text # "تلميح" أو "كشف"

    # 🛑 [1] فحص نوع المسابقة (يجب أن تكون عامة)
    if not is_public_competition(chat_id): 
        return

    # ⏳ [2] فحص التوقيت (أول 5 ثواني فقط)
    start_time = question_timers.get(chat_id, 0)
    if (time.time() - start_time) > 5:
        return 

    # 🔢 [3] فحص حد الاستخدام (5 مرات لكل سؤال)
    tracker_key = f"{chat_id}_{user_id}"
    current_usage = whisper_usage_limit.get(tracker_key, 0)
    if current_usage >= 5:
        return 

    # 💳 [4] فحص الكروت من جدولك (Supabase)
    res = supabase.table("users_global_profile").select("cards_inventory").eq("user_id", user_id).execute()
    if not res.data: return
    inventory = res.data[0].get('cards_inventory', {"letter": 0, "reveal": 0})

    # 🎯 [5] تجهيز المحتوى بناءً على الأمر
    correct_ans = current_questions.get(chat_id)
    if not correct_ans: return

    if cmd == "تلميح":
        if inventory.get('letter', 0) <= 0:
            return await message.reply("⚠️ رصيد كروت (letter) لا يكفي!")
        content = get_whisper_hint(correct_ans)
        inventory['letter'] -= 1
        btn_label = "📩 فتح التلميح الذكي"
    
    elif cmd == "كشف":
        if inventory.get('reveal', 0) <= 0:
            return await message.reply("⚠️ رصيد كروت (reveal) لا يكفي!")
        content = get_whisper_reveal(correct_ans)
        inventory['reveal'] -= 1
        btn_label = "🔓 فتح كشف الإجابة"

    # 🔄 [6] تحديث الكروت في قاعدة البيانات
    supabase.table("users_global_profile").update({"cards_inventory": inventory}).eq("user_id", user_id).execute()

    # 💾 [7] حفظ الهمسة وإرسال الزر
    cached_whispers[f"{chat_id}_{user_id}"] = content # الحفظ مرتبط بالروم واللاعب
    whisper_usage_limit[tracker_key] = current_usage + 1
    
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton(text=btn_label, callback_data=f"show_whp:{user_id}"))
    
    sent_msg = await message.reply(
        f"👤 **{message.from_user.first_name}**\nاستخدمت همسة ({current_usage+1}/5) ⚡️", 
        reply_markup=markup,
        parse_mode="Markdown"
    )

    # إضافة ID الرسالة لقائمة الحذف عند انتهاء السؤال
    if chat_id not in active_whisper_msgs: active_whisper_msgs[chat_id] = []
    active_whisper_msgs[chat_id].append(sent_msg.message_id)
...
# ==========================================
# 4. قسم معالجات الأوامر (Commands)
# ==========================================
# أ- معالج الأمر النصي (بدون شرطة)
@dp.message_handler(lambda message: message.text == "صفقاتي")
async def cmd_view_trades_text(message: types.Message):
    user_id = message.from_user.id
...
@dp.message_handler(lambda message: message.text in ['تداول', 'السوق'])
async def listener_market(message: types.Message):
    user_id = message.from_user.id
    
    # جلب أفضل العملات (الأكثر رواجاً)
    ...
    # استدعاء الكيبورد المؤمن بالآيدي
...
async def listener_wallet(message: types.Message):
    ...
    # استخدام الكيبورد المؤمن بآيدي المستخدم والدين
    await message.answer(text, reply_markup=get_wallet_keyboard(user_id, debt), parse_mode="HTML")
    
 
 # ب- معالج الكولباك (عند الضغط على أزرار التنقل)
@dp.callback_query_handler(lambda c: c.data.startswith('active_trades_view'))
async def callback_view_trades(callback_query: types.CallbackQuery):
    user_id = int(callback_query.data.split(':')[1]) if ':' in callback_query.data else callback_query.from_user.id
    trades, text = await get_active_trades_report(user_id)
...
@dp.callback_query_handler(lambda c: True) # معالج شامل للأزرار
async def global_callback_guard(callback_query: types.CallbackQuery):
    data_parts = callback_query.data.split(':')
    
    # فحص إذا كان الزر يحتوي على آيدي مستخدم (موقع الآيدي غالباً الثاني)
    # مثال: market_tab:12345:trending
    if len(data_parts) > 1 and data_parts[1].isdigit():
        owner_id = int(data_parts[1])
        if callback_query.from_user.id != owner_id:
            return await callback_query.answer("⚠️ هذي الواجهة ليست لك! افتح محفظتك الخاصة 🤫", show_alert=True)

    # هنا تكمل توجيه البيانات للمعالجات الأخرى...
    # (يمكنك استخدام if/elif لتوزيع الأوامر)
# ==========================================
# 5. قسم معالجات الأزرار (Callbacks) - المحرك الأساسي
# ==========================================

@dp.callback_query_handler(lambda c: c.data.startswith('coin_view:'))
async def process_coin_view(callback_query: types.CallbackQuery):
    ...
    # تحليل احترافي للمؤشرات حسب إعداداتك (RSI 78/22, EMA 50)
    ...
    text += f"• <b>EMA 50 (الأزرق):</b> {ema50:,.4f} ({ema_status})\n"
    text += f"• <b>RSI (78/22):</b> {rsi:.1f} ({rsi_status})\n"
    text += f"• <b>Bollinger (الأبيض):</b> {float(coin.get('bb_middle', price)):,.4f}\n"
    ...
    await bot.edit_message_text(text, callback_query.message.chat.id, callback_query.message.message_id, 
                                reply_markup=get_coin_keyboard(symbol), parse_mode="HTML")

@dp.callback_query_handler(lambda c: c.data.startswith('setup_trade:'))
async def process_setup_trade(callback_query: types.CallbackQuery):
    user_id = callback_query.from_user.id
    _, symbol, side = callback_query.data.split(':')
    
    # جلب السعر لإنشاء الجلسة
    ...
    # تهيئة جلسة التداول في الرام
    trade_sessions[user_id] = {
        'symbol': symbol,
        'side': side,
        'entry_price': price,
        'leverage': 10,       # افتراضي
        'margin_pct': 25,     # افتراضي
        'duration': '4h',     # افتراضي
        'balance': balance
    }
...
@dp.callback_query_handler(lambda c: c.data.startswith('trade_cycle:'))
...
@dp.callback_query_handler(lambda c: c.data.startswith('dca_trade:'))
async def process_dca_trade(callback_query: types.CallbackQuery):
    # فك التشفير: dca_trade:{user_id}:{trade_id}
    data = callback_query.data.split(':')
    user_id = int(data[1])
    trade_id = data[2]

    # 1. جلب بيانات الصفقة الحالية
    res = supabase.table("active_trades").select("*").eq("id", trade_id).execute()
    if not res.data:
        return await callback_query.answer("⚠️ الصفقة غير موجودة أو مغلقة.")
    ...
    # 2. جلب سعر العملة الحالي (للمتوسط الجديد)
    coin_res = supabase.table("crypto_market_simulation").select("current_price").eq("symbol", symbol).execute()
    current_price = float(coin_res.data[0]['current_price'])

    # 3. جلب رصيد البنك للتأكد من قدرة المستخدم على التعزيز
    bank_bal = await get_user_bank_balance(user_id)
    dca_amount = old_margin  # سنفترض أن التعزيز بنفس مبلغ الهامش الحالي (Double Down)
    ...
    # ب- السعر المتوسط الجديد (Equation: Total Cost / Total Units)
    # الوحدات القديمة = (الهامش * الرافعة) / سعر الدخول القديم
    old_units = (old_margin * leverage) / old_entry
    # الوحدات الجديدة = (مبلغ التعزيز * الرافعة) / السعر الحالي
    ...
    # ج- سعر التصفية الجديد (بناءً على المتوسط الجديد)
    new_liquidation = calculate_liquidation(new_entry_price, leverage, side)

    # 4. تحديث سوبابيس (خصم من البنك وتحديث الصفقة)
    # تحديث البنك
    ...
    # التعديل الصحيح لاستدعاء دالة التحديث الموجودة في ملفك
    await callback_view_trades(callback_query)
    
    
@dp.callback_query_handler(lambda c: c.data.startswith('close_trade:'))
async def handle_manual_close_request(callback_query: types.CallbackQuery):
    data = callback_query.data.split(':')
    user_id = int(data[1])
    trade_id = data[2]
    
    # جلب السعر اللحظي للإغلاق
    ...
    if success:
        ...
        # استدعاء دالة العرض لتحديث القائمة (تأكد أن الاسم مطابق لدالتك)
        await callback_view_trades(callback_query) 
    else:
        await callback_query.answer("❌ فشل إغلاق الصفقة.")
            
@dp.callback_query_handler(lambda c: c.data.startswith('exec_loan:'))
async def exec_loan(callback_query: types.CallbackQuery):
    # فك التشفير: exec_loan:{user_id}:{amount}
    data = callback_query.data.split(':')
    user_id = int(data[1])
    loan_amount = float(data[2])
    
    # 1. جلب بيانات اللاعب الحالية للتأكد من تحديثها بدقة
    user_data = await get_user_data(user_id)
    if not user_data:
        return await callback_query.answer("❌ خطأ: لم يتم العثور على ملف اللاعب.")

    # 2. حساب القيم الجديدة
    current_bank = float(user_data.get('bank_balance', 0))
    current_debt = float(user_data.get('debt_balance', 0))
    
    new_bank = current_bank + loan_amount
    new_debt = current_debt + loan_amount

    # 3. تحديث "ملف اللاعب" في سوبابيس (Update Profile)
    try:
        supabase.table("users_global_profile").update({
            "bank_balance": new_bank,
            "debt_balance": new_debt,
            "last_loan_date": datetime.now().isoformat() # توثيق وقت القرض
        }).eq("user_id", user_id).execute()
        
        # 4. إشعار النجاح وتحديث واجهة المحفظة
        # إشعار النجاح
        await callback_query.answer(f"✅ تم إيداع {loan_amount:,.2f} $ بنجاح.", show_alert=True)
        
        # بدلاً من إرسال رسالة جديدة، نقوم باستدعاء دالة المحفظة لتعديل الرسالة القائمة
        # تأكد أن دالة المحفظة عندك تدعم التعديل أو استدعيها مباشرة
        await listener_wallet(callback_query.message)
        
        
    except Exception as e:
        logging.error(f"Error in loan execution: {e}")
        await callback_query.answer("❌ فشل تحديث ملف اللاعب، حاول لاحقاً.")
        
        
@dp.callback_query_handler(lambda c: c.data.startswith('trade_confirm:'))
async def process_trade_confirm(callback_query: types.CallbackQuery):
    ...
    try:
        ...
        # 2. تسجيل الصفقة في الجدول (كما طلبت في SQL)
        ...
        text += f"المبلغ المحجوز (الهامش): {margin_amount:,.2f} $\n"
        text += f"رصيدك المتبقي: {new_balance:,.2f} $"
        
        # تأمين أزرار ما بعد التنفيذ (Success View)
        markup = InlineKeyboardMarkup()
        
        # ربط عرض الصفقات والعودة للسوق بالآيدي لمنع المبعسسين من التلصص على صفقاتك
        markup.add(
            InlineKeyboardButton("📋 عرض صفقاتي", callback_data=f"active_trades_view:{user_id}")
        )
        markup.add(
...
# ==========================================
# 1. إعدادات الجلسات والقيم الثابتة
# ==========================================
trade_sessions = {}
LEVERAGE_LEVELS = [1, 5, 10, 20, 50, 100]
MARGIN_PCT_LEVELS = [5, 10, 25, 50, 75, 100]
...
# --- نظام التحويل الكتابي ---
@dp.callback_query_handler(lambda c: True)
async def global_callback_handler(callback_query: types.CallbackQuery):
    data = callback_query.data.split(':')
    
    # إذا كان الزر يحتوي على آيدي (مثلاً في العنصر الثاني)
    if len(data) > 1 and data[1].isdigit():
        target_user_id = int(data[1])
        if callback_query.from_user.id != target_user_id:
            return await callback_query.answer("🚫 هذي ليست محفظتك! العب بعيد يا مبعسس 🤫", show_alert=True)

    # هنا نقوم بتوجيه الأوامر حسب النوع
    if data[0] == 'setup_trade':
        await process_setup_trade(callback_query)
    elif data[0] == 'trade_cycle':
        await process_trade_cycle(callback_query)

@dp.callback_query_handler(lambda c: c.data.startswith('transfer_flow:'))
async def transfer_init(callback_query: types.CallbackQuery):
    ...
    trade_sessions[f"wait_trans_{callback_query.from_user.id}"] = direction
    await callback_query.answer()

@dp.message_handler(lambda m: m.reply_to_message and "أدخل المبلغ المراد" in m.reply_to_message.text)
...
# --- نظام الاقتراض والتصفير ---

@dp.callback_query_handler(lambda c: c.data == "loan_menu")
async def loan_menu(callback_query: types.CallbackQuery):
    ...
    data = await get_user_data(user_id)
    max_loan = float(data['bank_balance']) * 0.5 # حد القرض نصف الرصيد

    # إعداد واجهة القرض بتأمين الآيدي
    markup = InlineKeyboardMarkup()
    
    # ربط عملية تنفيذ القرض بالآيدي لمنع المبعسسين من توريطك بـ ديون
    markup.add(
        InlineKeyboardButton(
            f"💰 اقتراض {max_loan:,.2f} $ (قرض ذكي)", 
            callback_data=f"exec_loan:{user_id}:{max_loan}"
        )
    )
    
    # العودة للمحفظة أيضاً مؤمنة بالآيدي
    markup.add(
        InlineKeyboardButton("🔙 عودة للمحفظة", callback_data=f"wallet_view:{user_id}")
    )
    
    # رسالة الواجهة مع تفاصيل المبلغ المتاح
...
@dp.callback_query_handler(lambda c: c.data.startswith('close_trade:'))
async def handle_manual_close_request(callback_query: types.CallbackQuery):
    # فك التشفير: close_trade:{user_id}:{trade_id}
    data = callback_query.data.split(':')
    user_id = int(data[1])
    trade_id = data[2]
    
    # جلب السعر الحالي من السوق لإغلاق الصفقة عليه
    res = supabase.table("active_trades").select("symbol").eq("id", trade_id).execute()
    if not res.data: return await callback_query.answer("⚠️ الصفقة مغلقة بالفعل.")
    ...
    if success:
        await callback_query.answer(f"✅ تم الإغلاق! الربح/الخسارة: {pnl:+.2f} $", show_alert=True)
        await callback_view_trades(callback_query) # تحديث القائمة فوراً
    else:
        await callback_query.answer("❌ فشل إغلاق الصفقة.")
...
async def exec_loan(callback_query: types.CallbackQuery):
    ...
    # تحديث الرصيد والدين معاً
    supabase.rpc('increment_bank_and_debt', {'u_id': user_id, 'amount': amount}).execute()
    
    await callback_query.answer("✅ تمت إضافة القرض لحساب التداول!", show_alert=True)
    await cmd_wallet(callback_query.message)
...
# --- [ معالج الكروت ] ---
# ==========================================
@dp.callback_query_handler(lambda c: c.data and c.data.startswith('show_whp:'))
async def process_whisper_click(callback_query: types.CallbackQuery):
    owner_id = int(callback_query.data.split(":")[1])
    chat_id = callback_query.message.chat.id
    
    # ❌ منع التطفل
    if callback_query.from_user.id != owner_id:
        return await callback_query.answer("⚠️ هذه الهمسة ليست لك!", show_alert=True)
    
    # ✅ جلب المحتوى وعرضه كـ Alert
    content = cached_whispers.get(f"{chat_id}_{owner_id}")
    if content:
        await callback_query.answer(text=content, show_alert=True)
    else:
        await callback_query.answer(text="❌ انتهت صلاحية الهمسة!", show_alert=True)
        ...
        for i, q in enumerate(selected_questions):
            ...
            for cid in chats_to_broadcast:
                active_quizzes[cid].update({
                    ...
                    "mode": quiz_data.get('mode', 'السرعة ⚡') # 👈 [ الحل السحري ]: تمرير النمط للرام!
                })         

            # --- [ هـ ] تجهيز التلميح الذكي (Smart Hint) ---
            # --- [ هـ ] إعداد بيانات الهمسات للمستمعات (بدلاً من التلميح القديم المجاني) ---
            # نقوم بتسجيل الإجابة ووقت البدء لكل مجموعة لكي يعرف المستمع متى يسمح بالهمسة
            for cid in chats_to_broadcast:
                question_timers[cid] = time.time()
                current_questions[cid] = str(ans).strip()

            # --- [ و ] إرسال السؤال للمجموعات (البث المتزامن) ---
            send_tasks = []
            for cid in chats_to_broadcast:
                # استدعاء "المايسترو" لإرسال السؤال بالنمط المحدد (تم حذف التلميح من هنا)
                task = send_quiz_master(
                    ...
                    settings={
                        ...
                        'quiz_style': current_style,
                        'is_public': True
                        # تم حذف 'smart_hint' و 'normal_hint' بالكامل
                    },
            ...
            # 💡 في هذه الأثناء.. المستمعات (Handlers) التي برمجناها ستلتقط أوامر 
            # "تلميح" أو "كشف" من اللاعبين بشكل غير متزامن دون إيقاف هذه الحلقة
            ...
            # --- [ ح ] تنظيف مخلفات الهمسات فور الانتهاء ---
            # بمجرد أن يكسر الكود حلقة الانتظار (إما لانتهاء الوقت أو فوز شخص)، نمسح الهمسات
            for cid in chats_to_broadcast:
                await cleanup_round_whispers(cid)
